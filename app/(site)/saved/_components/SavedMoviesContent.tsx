...
export function SavedMoviesContent(
  ...
  const play = useCallback(async (id: string) => {
    ...
    if (!url) { alert("Please save the movie again — its data was cleared."); return; }
    const tab = window.open("", "_blank");
    if (tab) tab.document.write(`<!doctype html><html><body style="margin:0;background:#000"><video src="${url}" autoplay controls style="width:100vw;height:100vh"></video></body></html>`);
  }, []);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
      ...
      {items.length === 0 ? (
        <p className="text-sm text-[color:var(--color-text-secondary)]">Nothing saved yet. Open a movie and click <strong>+ Save</strong> to keep it for offline viewing.</p>
      ) : (

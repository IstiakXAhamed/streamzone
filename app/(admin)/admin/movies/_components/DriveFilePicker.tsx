...
/** Button that lets the admin pick a video from their PC, uploads it directly
 *  to Drive via the resumable-upload pipeline, and reports the resulting fileId. */
export function DriveFilePicker({ label, onPicked }: Props) {
  ...
  return (
    <div className="flex items-center gap-2">
      <label className="flex h-10 cursor-pointer items-center gap-2 rounded-xl bg-[color:var(--color-surface-2)] px-3 text-sm text-[color:var(--color-text-primary)]">
        ...
        <input ref={inputRef} type="file" accept="video/*,image/*" className="hidden" onChange={handleFile} disabled={busy} />
      </label>
